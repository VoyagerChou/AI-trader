...
def _classify_agent_intent(message: str, attachments: list[dict], context: dict, language: str) -> dict:
    ...
    system_prompt = (
        "You are the AI-Trader Agent Intent Router. Classify the user's message into a "
        ...
        "For strategy creation, prefer AI-Trader native workflows: indicator_ide for chart/backtest "
...
def _build_system_prompt(language: str, context: dict, intent: str, has_image: bool, json_response: bool = True) -> str:
    ...
    base = (
        "You are AI-Trader Copilot, a trading system assistant for open-source quant users. "
        ...
        "If the user asks to write strategy code, stay inside AI-Trader native workflows. "
...
def _register_report_pdf_font(prefer_cjk: bool = False) -> str:
    ...
    for path, is_cjk in candidates:
        ...
        try:
            if Path(path).exists():
                pdfmetrics.registerFont(TTFont("AI-TraderSans", path))
                return "AI-TraderSans"
        except Exception as e:
...
def _build_ai_report_pdf(report: dict, target: dict | None = None, language: str = "en-US") -> bytes:
    ...
    labels = {
        "title": "AI-Trader AI Analysis Report" if not zh else "AI-Trader AI 分析报告",
    ...
    def footer() -> None:
        ...
        c.drawRightString(width - margin, 12 * mm, f"AI-Trader · {page_no}")
...
def export_chat_report_pdf():
    ...
    filename = f"AI-Trader_{symbol or 'report'}_{_now_utc().strftime('%Y%m%d')}.pdf"
    return Response(
        pdf_bytes,
