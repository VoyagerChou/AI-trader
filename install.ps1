# AI-Trader interactive installer for Windows PowerShell.
#
# Usage:
#   irm https://raw.githubusercontent.com/brokermr810/AI-Trader/main/install.ps1 | iex
#
# Optional environment overrides:
#   $env:AI_TRADER_INSTALL_REF = "main"
#   $env:AI_TRADER_INSTALL_DIR = "C:\AI-Trader"

$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

$InstallDir = if ($env:AI_TRADER_INSTALL_DIR) { $env:AI_TRADER_INSTALL_DIR } else { Join-Path $HOME "ai-trader" }
$InstallRef = if ($env:AI_TRADER_INSTALL_REF) { $env:AI_TRADER_INSTALL_REF } else { "main" }
$GithubRaw = "https://raw.githubusercontent.com/brokermr810/AI-Trader/$InstallRef"
...
function Check-Prerequisites {
    Write-Host "AI-Trader installer" -ForegroundColor Blue
...
function Collect-Settings {
    $script:AdminUser = Read-Value "Admin username" ((Get-EnvValue $BackendEnv "ADMIN_USER") -replace '^$', 'ai-trader')
    ...
    if ($existingSecret -and $existingSecret -ne "ai-trader-secret-key-change-me") {
...
function Print-Summary {
    $apiPort = ($BackendPort -split ':')[-1]
    Write-Host ""
    Write-Host "AI-Trader is ready." -ForegroundColor Green
    Write-Host ""
