#!/usr/bin/env bash
#
# AI-Trader interactive installer for Linux and macOS.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/brokermr810/AI-Trader/main/install.sh | bash
#
# Custom install directory:
#   curl -fsSL https://raw.githubusercontent.com/brokermr810/AI-Trader/main/install.sh | bash -s -- /opt/ai-trader
#
# Optional environment overrides:
#   AI_TRADER_INSTALL_REF=main
#   AI_TRADER_INSTALL_DIR=/opt/ai-trader
#
...
INSTALL_DIR="${1:-${AI_TRADER_INSTALL_DIR:-$HOME/ai-trader}}"
INSTALL_REF="${AI_TRADER_INSTALL_REF:-main}"
GITHUB_RAW="https://raw.githubusercontent.com/brokermr810/AI-Trader/${INSTALL_REF}"
...
check_prerequisites() {
    say "${BLUE}AI-Trader installer${NC}"
...
collect_settings() {
    ...
    ADMIN_USER_VALUE=$(read_line "Admin username" "${existing_user:-ai-trader}")
    ...
    if [ -n "$existing_secret" ] && [ "$existing_secret" != "ai-trader-secret-key-change-me" ]; then
...
print_summary() {
    say ""
    say "${GREEN}AI-Trader is ready.${NC}"
    say ""
