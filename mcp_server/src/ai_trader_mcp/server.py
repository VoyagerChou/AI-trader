"""
AI-Trader MCP server — exposes the Agent Gateway as MCP tools.
...
def _env(name: str, required: bool = True) -> str:
    ...
    if not value and required:
        print(
            f"[ai-trader-mcp] missing required env var: {name}",
            file=sys.stderr,
        )
        sys.exit(2)
    return value
...
BASE_URL = _env("AI_TRADER_BASE_URL").rstrip("/")
AGENT_TOKEN = _env("AI_TRADER_AGENT_TOKEN")
TIMEOUT_S = float(os.environ.get("AI_TRADER_TIMEOUT_S", "60"))
JOB_STREAM_MAX_EVENTS = int(os.environ.get("AI_TRADER_MCP_JOB_STREAM_MAX_EVENTS", "200"))
JOB_STREAM_MAX_SECONDS = float(os.environ.get("AI_TRADER_MCP_JOB_STREAM_MAX_SECONDS", "300"))
JOB_POLL_MAX_SECONDS = float(os.environ.get("AI_TRADER_MCP_JOB_POLL_MAX_SECONDS", "300"))
...
mcp = FastMCP(
    "ai-trader",
    instructions=(
        "Tools for the AI-Trader self-hosted quant platform. "
...
def get_indicator_authoring_contract() -> Any:
    """Fetch AI-Trader indicator I/O contract + starter Python template.
...
def _resolve_transport() -> str:
    raw = (os.environ.get("AI_TRADER_MCP_TRANSPORT") or "stdio").strip().lower()
    if raw in ("http", "streaming-http", "streamable_http"):
        raw = "streamable-http"
    if raw not in _TRANSPORTS:
        print(
            f"[ai-trader-mcp] unknown transport '{raw}'. "
...
def _apply_http_settings_from_env() -> None:
    host = (os.environ.get("AI_TRADER_MCP_HOST") or "").strip()
    port_raw = (os.environ.get("AI_TRADER_MCP_PORT") or "").strip()
    ...
    if port_raw:
        try:
            ...
            print(
                f"[ai-trader-mcp] invalid AI_TRADER_MCP_PORT='{port_raw}', ignoring.",
                file=sys.stderr,
            )
...
def main() -> None:
    ...
    Transport selection (env-only — works in both desktop and cloud):
      AI_TRADER_MCP_TRANSPORT=stdio              (default; stdin/stdout)
      AI_TRADER_MCP_TRANSPORT=sse                (SSE over HTTP)
      AI_TRADER_MCP_TRANSPORT=streamable-http    (newer MCP HTTP transport)
      AI_TRADER_MCP_HOST=0.0.0.0                 (bind for HTTP transports)
      AI_TRADER_MCP_PORT=7800                    (port for HTTP transports)
    """
