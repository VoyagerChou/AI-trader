...
def fresh_module(monkeypatch):
    ...
    monkeypatch.setenv("AI_TRADER_BASE_URL", "http://localhost:8888")
    monkeypatch.setenv("AI_TRADER_AGENT_TOKEN", "qd_agent_test_token")
    sys.modules.pop("ai-trader_mcp.server", None)
    ...
    return importlib.import_module("ai-trader_mcp.server")


def test_transport_default_is_stdio(monkeypatch, fresh_module):
    monkeypatch.delenv("AI_TRADER_MCP_TRANSPORT", raising=False)
...
def test_transport_aliases_resolve(monkeypatch, fresh_module, raw, expected):
    monkeypatch.setenv("AI_TRADER_MCP_TRANSPORT", raw)
    assert fresh_module._resolve_transport() == expected


def test_transport_unknown_exits(monkeypatch, fresh_module):
    monkeypatch.setenv("AI_TRADER_MCP_TRANSPORT", "bogus-transport")
...
def test_apply_http_settings_without_error(monkeypatch, fresh_module):
    ...
    monkeypatch.setenv("AI_TRADER_MCP_HOST", "0.0.0.0")
    monkeypatch.setenv("AI_TRADER_MCP_PORT", "7777")
